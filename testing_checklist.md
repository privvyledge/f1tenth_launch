...
- [!] **Particle cloud visible in RViz** — load a `PoseArray` topic `/particlecloud`. Should see particle cloud spread across robot starting area. **[2026-08-04] Root cause confirmed, RViz check still pending**: the topic is `/particle_cloud` and its type is **`nav2_msgs/msg/ParticleCloud`**, NOT `PoseArray` — so the display must be added via **Add → By display type → nav2_rviz_plugins → Particle Cloud**. Verified publishing at 8.97 Hz alongside `/amcl_pose` at 8.68 Hz (headless, so nothing was rendered). Convergence-under-motion still untested. **[2026-08-27] Display added successfully and still rendered nothing — TWO blockers, both now identified, neither cleared, so this stays `[!]`.** **(1) QoS.** `nav2_rviz_plugins` is installed and the display subscribed — `ros2 topic info /particle_cloud --verbose` read `Subscription count: 1` and that subscriber *was* rviz2 — but **AMCL publishes `BEST_EFFORT` while the RViz display defaults to `RELIABLE`**, an incompatible pair that forms no delivery. Set the display's **Reliability Policy to Best Effort**. Read the *reliability* of both ends, never the subscriber count: the count reads 1 in the working and the broken case alike, exactly as with bug-238's durability. **(2) The motion gate, which is why this cannot be closed parked.** `/particle_cloud` was silent across 14 s even while the car was carried by hand: AMCL gates on *odometry* motion and `odometry/local` moved **0.000 m** through the carry, because without the VESC its only sources are rf2o and VSLAM and a lifted car gives neither a usable scan delta nor VSLAM tracking. The 8.97 Hz reading above is therefore not contradicted — it came from a run where odometry was genuinely moving. **To close:** on a drive session, display added by type with Reliability = Best Effort, and confirm particles both render and converge as the car moves.
